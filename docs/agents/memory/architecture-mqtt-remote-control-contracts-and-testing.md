...
1. `absorb/<slug>/sleep_timer/set` supports both permissive raw inputs (`'15'`, `15`, `'cancel'`, `'end_of_chapter'`) and structured JSON payloads (`{"duration_minutes": 15}`, `{"cancel": true}`, `{"mode": "end_of_chapter"}`).
...
- Sleep timer set commands accept raw strings/numbers or JSON objects (`{"duration_minutes": N}`, `{"mode": "end_of_chapter"}`, `{"cancel": true}`).
